...
export default function App() {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      await fetch(`${EVO_URL}/instance/logout/bot_oficial_v3`, {
        method: 'DELETE',
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        const res = await fetch(`${EVO_URL}/instance/connect/bot_oficial_v3`, { 
          method: "GET", 
          headers: { "apikey": API_KEY } 
        });
        ...
        if (data.instance && data.instance.state === "open") {
          ...
          try {
            const fetchRes = await fetch(`${EVO_URL}/instance/fetchInstances?instanceName=bot_oficial_v3`, {
